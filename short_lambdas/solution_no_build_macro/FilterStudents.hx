...
class FilterStudents {
	...
	static function main() {
		...
		var firstNamesStartingWithA = students.map(f(_.first)).filter(f(_.startsWith("A")));
		var studentsWhoPassed = students.filter(f(_.grade>50));
		var girls = students.filter(f(_.gender.match(Female)));
		var boys = students.filter(f(_.gender.match(Male)));
